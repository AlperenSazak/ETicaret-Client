...
import { OrderDetailDialogState, OrderDetailDialogsComponent } from 'src/app/dialogs/order-detail-dialogs/order-detail-dialogs.component';
  ...
  @ViewChild(MatPaginator) paginator: MatPaginator;
   ...
   async getOrders() {
    ...
    const allOrders : { totalOrderCount: number; orders: List_Order[] } = await 
      this.orderService.getAllOrders(this.paginator ? 
      this.paginator.pageIndex : 0, this.paginator ? this.paginator.pageSize : 5, () => this.hideSpinner
      (SpinnerType.Timer), (errorMessage: any) => {
      this.alertyfyService.message(errorMessage.message, {
        dismissOthers: true,
